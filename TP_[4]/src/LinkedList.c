...
static Node* getNode(LinkedList* this, int nodeIndex)
{
	...
	{
		pNodo = this->pFirstNode; // pNodo apunta a nodo 0.
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
	{
		...
		if(nuevoNodo != NULL)
		{
			//hacer que el nuevo nodo apunte al elemento
			nuevoNodo->pElement = pElement;
			nuevoNodo->pNextNode = getNode(this, nodeIndex);

			//enganchar el nodo en el LL
			if(nodeIndex == 0)
			{
...
{
	int returnAux = -1;
	Node* pNodeTodoDelete = NULL;
	Node* pNodeAdelante = NULL;
	if(this != NULL && index >= 0 && index < ll_len(this))
	{
		if(index == 0)
		{
			pNodeTodoDelete = this->pFirstNode;
			this->pFirstNode =	pNodeTodoDelete->pNextNode;
		}
		else
		{
			pNodeAdelante = getNode(this, index -1);
			pNodeTodoDelete = pNodeAdelante->pNextNode;
			pNodeAdelante->pNextNode = pNodeTodoDelete->pNextNode;
		}
		free(pNodeTodoDelete);
		this->size--;
		returnAux = 0;
	}
	return returnAux;
